...
const JoinPlanB = (props) => {
  ...
  const onChange = (value) => {
    fetch(
      `http://api.allorigins.win/get?url=https://api.mojang.com/users/profiles/minecraft/${value}`
    )
  ...
  const refferedApplicant = () => {
    return (
      <div>
        <img
          src={`https://crafatar.com/renders/body/${avatar}?size=120&overlay`}
        />
        <form name="Application" data-netlify="true" action="/thankyou">
          ...
          <input
            ...
            disabled
            hidden
            value={`${params.discord}#${params.discrim}`}
            name="discord"
          />
